...
std::string ID_Table::sensorType2Str(uint8_t sensorType) {
	return sensorType2Str(static_cast<SensorType>(sensorType));
}
std::string ID_Table::sensorType2Str(SensorType sensorType) {
	std::string sensTypeStr;
	if (sensorType == ID_Table::SensorType::TEMP) {
		sensTypeStr = "TEMP";
	} else if (sensorType == ID_Table::SensorType::HUMIDITY) {
		sensTypeStr = "HUMI";
	} else if (sensorType == ID_Table::SensorType::PRESS) {
		sensTypeStr = "PRES";
	}
	return sensTypeStr;
}
