<?php
# $Id$
# Testing phplot - Lines
  require_once 'phplot.php';
  
$data = array(
  array('eins', -4, -64,  16,  40),
  array('zwei', -3, -27,   9,  30),
  array('drei', -2,  -8,   4,  20),
  array('vier', -1,  -1,   1,  10),
  array('fuenf',  0,   0,   0,   0),
  array('sechs',  1,   1,   1, -10),
  array('sieben',  2,   8,   4, -20),
  array('acht',  3,  27,   9, -30),
  array('neun',  4,  64,  16, -40),
);
  
$p = new PHPlot();
$p->SetTitle('PlotType: lines, DataType: data-data');
$p->SetDataType('data-data');
$p->SetDataValues($data);

$p->SetPrintImage(False); // No automatic output

# We don't use the data labels (all set to '') so might as well turn them off:
//$p->SetXDataLabelPos('none');
$p->SetXDataLabelPos('plotdown');

# Need to set area and ticks to get reasonable choices.
$p->SetPlotAreaWorld(-4, -70, 4, 80);
$p->SetXTickIncrement(1);
$p->SetYTickIncrement(10);

# Don't use dashes for 3rd line:
$p->SetLineStyles('solid');

# Make the lines thicker:
$p->SetLineWidths(3);

# Draw both grids:
$p->SetDrawXGrid(True);
$p->SetDrawYGrid(True);

# And a legend:
$p->SetLegend(array('x^3', 'x^2', '-10x'));

$p->SetPlotType('lines');
$p->DrawGraph();


?><!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"
     "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<title>PHPlot Example - Inline Image</title>
</head>
<body>
<h1>PHPlot Example - Inline Image</h1>
<p>This is a plot of sin() and cos().</p>
<img src="<?php echo $p->EncodeImage();?>" alt="Plot Image">
<input type="button" value="Say Hi!" onclick="location='lines3_second.php'" />
</body>
</html>

